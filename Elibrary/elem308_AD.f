...
!     AD variables
      ...
      type(dType)  :: Cdet, Inv1, K1, Fdet, Ftmp_det
      type(dType), dimension(3,3) ::  Fdef, Ctens, dWdC, dJdC, Sigma
      type(dType), dimension(3,3) :: SecPK, Cinv, Finv, FS, K2, Ftmp
...
c       compute the deformation gradient at Gauss Point
        ...
        do inode = 1,nnode
          do j = 1,ndim
            do i = 1,ndim
              Ftmp(i,j)=Ftmp(i,j)-uelem_ad(i,inode)*shap(j,inode)
            end do
          end do
        end do
...
c       create element residual for right hand side
        ...
        do i = 1, ndim
c         do j = 1, ndim
c          Tp(i) = Tp(i) + 2.0d0*temp*dJdC(j,i)*pres(j)
