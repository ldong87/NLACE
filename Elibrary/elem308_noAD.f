...
c**********************************************************
      ...
      double precision Sigma_no_pres(3,3)
...
c       compute the deformation gradient at Gauss Point
     ...
     $             Ftmp(1,1)*Ftmp(2,3)*Ftmp(3,2)
        ...
        if (Ftmp_det .lt. 1.0d-2) then
          Print*,"elem305.f: I-du/dx is negative at element", ielem,
     $           "  Jacob ", Ftmp_det," ... exiting"
c          stop
        endif
...
c        endif
        ...
        Ctang(:,:,:,:)=Ctang(:,:,:,:)-0.0d0*pres(4)*d2JdC(:,:,:,:)
...
c       create the b-matrix
        ...
        do i = 1, ndim
         do t = 1, ndim
          do k = 1, ndim
           do j = 1, ndim
            Sigma_no_pres(i,t) = Sigma_no_pres(i,t)+
     $       2.0d0/Fdet*Fdef(i,j)*dWdC(j,k)*Fdef(t,k)
           enddo
          enddo
         enddo
        enddo
     ...
     $                (shap(1,inode)*ident(1,idofn)+
     $                 shap(2,inode)*ident(2,idofn)+
     $                 shap(3,inode)*ident(3,idofn))
     $                                         *shap(4,jnode)*wtjac
c     $                 shap(idofn,inode)*shap(4,jnode)*wtjac
...
c                  end do
                  do j = 1, ndim
                   do t = 1, ndim
                    estif(ievab,jevab)=estif(ievab,jevab)+
     $               shap(j,inode)*
     $               ( -Fdef(t,jdofn)*Sigma_no_pres(idofn,j)
     $                 +Fdef(idofn,jdofn)*Sigma_no_pres(t,j)
     $                 +Lmat(idofn,j,jdofn,t)
     $                 +Fdef(j,jdofn)*Sigma_no_pres(t,idofn) )
     $                *shap(t,jnode)*wtjac
                   enddo
                  enddo
