...
c     LD: points data matching, no integration points
c**********************************************************
      USE IOUNIT
      USE MAINMEM
      implicit none
 ...
 6    l  = int(pelem(1))
      ...
      do inode = 1, nnode
        do idofn = 1,4! 4 = elemvec_ndofn(inode)
          ...
          do jnode = 1,nnode
            jdofn=idofn
            ii = idnum(elnods(jnode),jdofn)
            if (ii.ne.0) then
              eforc(ievab) = eforc(ievab)-
     $              uelem_diff(jdofn,jnode)/
...
c     compute the gradient and the objective function
      ...
      do j=1,ndim
        do inode=1,nnode
          ...
          if (ii.ne.0) then
            udiff(j) = udiff(j)+uelem_diff(j,inode)/
     $             sqrt(float(nod_el_coun(ii+1)-nod_el_coun(ii))) 
          endif
        enddo
      enddo
      
      elemDataMatch(ielem) = elemDataMatch(ielem) + 0.5d0*(
