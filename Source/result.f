...
c     Print connectivity
     ...
     $           .and.(lrefn(ielem,1).ne.310)) then! skip surface elements
           nen = lrefn(ielem,2)
...
c          write(12,*)8,(lnods(ielem,inode)-1,inode=1,8)
     ...
     $            .or.(lrefn(ielem,1).eq.310)) then! count the surface elements
...
c       write the gradient for nodal data 
     ...
     $        (lrefn(ielem,1).ne.310))then! skip surface elements
