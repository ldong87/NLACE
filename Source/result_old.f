...
c     Print connectivity
      ...
      jelem = 0
      do ielem = 1,nelem
        if ((lrefn(ielem,1).ne.306).and.(lrefn(ielem,1).ne.611)) then! skip surface elements
          nen = lrefn(ielem,2)
...
c          write(12,*)8,(lnods(ielem,inode)-1,inode=1,8)
     ...
     $           " exiting"
      ...
      do ielem = 1,nelem
        nen = lrefn(ielem,2)
        if ((lrefn(ielem,1).eq.306).or.(lrefn(ielem,1).eq.611)) then! count the surface elements
...
c       write the gradient for nodal data 
     ...
     $       (lrefn(ielem,1).ne.611)) then! skip surface elements
