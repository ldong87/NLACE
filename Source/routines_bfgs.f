c================    L-BFGS-B (version 2.1)   ==========================
     ...
     +       task, iprint,  csave, lsave, isave, dsave, prefixOut)
...
c     ************
     ...
     +  csave,lsave,isave(22),dsave,prefixOut)
...
c======================= The end of setulb =============================
     ...
     +                  iprint, csave, lsave, isave, dsave, prefixOut)
...
c
c       Linpack Library ... dcopy, ddot_bfgs.
c
c
c     References:
c
...
c     ************
     ...
     +                 iword,nfree,nact,ileave,nenter
      double precision theta,fold,ddot_bfgs,dr,rr,tol,dpmeps,
...
c           'info' records the termination information.
         ...
         nF=1
 53      if (prefixOut(nF:nF).ne." ") then
            nF=nF+1
            goto 53
         endif
         prefixOut(nF:(nF+3))='.ite'
         if (iprint .ge. 1) then
c           open a summary file
            itfile = 8
            open (8,file=prefixOut(1:(nF+3)),status='unknown')
         endif            
         prefixOut(nF:(nF+3))='    '
...
c                                terminate the algorithm.
         ...
         print*,'Linf(proj_g) = ',sbgnrm,', pgtol = ',pgtol! JFD
         goto 999
      endif 
...
c       terminate the algorithm.
      ...
      if ((fold - f) .le. tol*ddum) then
        if (5.le.iter) then! JFD added to allow the algorithm to start with factr>0.0d0
c         terminate the algorithm.
          task = 'CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH'
          print*,'REL_R_O_F = ',(fold-f)/ddum,', factr = ',factr,! JFD
     $          ', epsmch = ',epsmch,'iter = ',iter! JFD
...
c     Compute d=newx-oldx, r=newg-oldg, rr=y'y and dr=y's.
      ...
      do 42 i = 1, n
         r(i) = g(i) - r(i)
  42  continue
      rr = ddot_bfgs(n,r,1,r,1)
      if (stp .eq. one) then  
         dr = gd - gdold
         ddum = -gdold
      else
         dr = (gd - gdold)*stp
         call dscal_bfgs(n,stp,d,1)
         ddum = -gdold*stp
      endif
...
c     Save local variables.
 ...
 1002 format (/,'  ++ At L-BFGS-B iterate',i5,4x,'f= ',1p,d12.5,
     +        4x,'|proj g|= ',1p,d12.5,' (cf *.ite)')
 ...
 1008 format (/, 
     ...
     +'   refresh the lbfgs memory and restart the iteration.')
 4001 format ('     Note: if iteration 0 leads to an error, try to ',
     +  'narrow the L-BFGS-B bounds or',/,'     try to change the ',
     +  'tolerance or try to increase the regularization',/)
...
c
c       Linpack ... dscal_bfgs dcopy, daxpy.
c
c
c     References:
c
...
c     ************
     ...
     +                 tu,tl,wmc,wmp,wmw,ddot_bfgs,tj,tj0,neggi,sbgnrm,
     +                 f2_org
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c                   complete the initialization of p for theta not= one.
         call dscal_bfgs(col,theta,p(col+1),1)
      endif
...
c           compute (wbp)Mc, (wbp)Mp, and (wbp)M(wbp)'.
	 ...
	 if (info .ne. 0) return
	 wmc = ddot_bfgs(col2,c,1,v,1)
	 wmp = ddot_bfgs(col2,p,1,v,1) 
	 wmw = ddot_bfgs(col2,wbp,1,v,1)
...
c     ************
     ...
     +                 col2,pbegin,pend,dbegin,dend,upcl
      double precision ddot_bfgs,temp1,temp2,temp3,temp4
      double precision one,zero
      parameter        (one=1.0d0,zero=0.0d0)
...
c        upper triangle of (2,2) block of wn.
      ...
      do 72 is = col+1, col2
         do 74 js = is, col2
            wn(is,js) = wn(is,js) + ddot_bfgs(col,wn(1,is),1,wn(1,js),1)
  74        continue
  72     continue
...
c     **********
      ...
      integer          i
      double precision ddot_bfgs,a1,a2
...
c                                             and the last column of SS:
      pointr = head
      do 51 j = 1, col - 1
	 sy(col,j) = ddot_bfgs(n,d,1,wy(1,pointr),1)
	 ss(j,col) = ddot_bfgs(n,ws(1,pointr),1,d,1)
         pointr = mod(pointr,m) + 1
  51  continue
      if (stp .eq. one) then
         ss(col,col) = dtd
...
c     ************
      ...
      if (iprint .ge. 0) then
c         write (6,7001)
c JFD         write (6,7001) epsmch
c JFD         write (6,*) 'N = ',n,'    M = ',m
         if (iprint .ge. 1) then
...
c JFD 7001 format (/,'  ++ RUNNING THE L-BFGS-B CODE',/)
c JFD     + /,'           * * *',/,/,
c JFD     + 'Machine precision =',1p,d10.3)
 7001 format (/,'  ++ RUNNING THE L-BFGS-B CODE',/,/,
     + '     Note: if iteration X0 leads to an error, try to increase ',
     + 'the number of loading steps or',/,'     try to increase the ',
     + 'tolerance or try to provide a better initialization ',
     + 'of the material',/,'     properties or try to impose ',
     + 'smoother boundary condition (if DBCs are enforced).',/)
...
c         if (imod .eq. 0) write (6,2001) iter,f,sbgnrm
 ...
 1004 format (/,a4, 1p, 6(1x,d11.4),/,(4x,1p,6(1x,d11.4)))
 2001 format (/,'  ++ At L-BFGS-B iterate',i5,4x,'f= ',1p,
     +        d12.5,4x,'|proj g|= ',1p,d12.5)
...
c====================== The end of dcopy ===============================
      ...
      double precision function ddot_bfgs(n,dx,incx,dy,incy)
c
...
c
         do 30 j = 1, n
            ...
            do 10 k = 1, jm1
               t = a(k,j) - ddot_bfgs(k-1,a(1,k),1,a(1,j),1)
               t = t/a(k,k)
               a(k,j) = t
               s = s + t*t
   10       continue
...
c====================== The end of dpofa ===============================
      ...
      subroutine  dscal_bfgs(n,da,dx,incx)
c
...
c     subroutines and functions
c
c     blas daxpy,ddot_bfgs
c     fortran mod
c
c     internal variables
c
      double precision ddot_bfgs,temp
      integer case,j,jj
c
...
c
   80    continue
            ...
            do 90 jj = 2, n
               j = n - jj + 1
               b(j) = b(j) - ddot_bfgs(jj-1,t(j+1,j),1,b(j+1),1)
               b(j) = b(j)/t(j,j)
   90       continue
  100       continue
         go to 140
...
c
  110    continue
            ...
            if (n .lt. 2) go to 130
            do 120 j = 2, n
               b(j) = b(j) - ddot_bfgs(j-1,t(1,j),1,b(1),1)
               b(j) = b(j)/t(j,j)
  120       continue
  130       continue
  140    continue
