          ...
          if (countFGcalls.eq.1) then ! first time through: finding balance with imposed bcs
            ...
            do while (accumStepL.lt.0.999999d0)
              ...
              do while (convRes.gt.tol)! Newton iterations
                ...
                call CSR_MATLAB (atang,jdiag,jcsr,aload,neqns,1)
...
c     Solve the primal problem (impose the BCs or increment the material properties)
      ...
      do while (accumStepL.lt.0.999999d0)
        ...
        if (accumStepL.gt.1.0d0) accumStepL=1.0d0! don't go beyond the limit
        bc_step = accumStepL
        if (lstep.eq.1) then ! this is to initialize primal with the inital guess
            call EnforceInitialCond(imeas)
        endif
c      print*,'something'
c      do ii=1,9
c      print*,(primal(ii,jj),jj=1,3)
c      enddo
        ...
        !  next do while loop for edbugging
c        do while (itNewton.lt.1)! to perform only one newton iteration of forward solve
          itNewton=itNewton+1
c          print*,'Printing Newton Iteration '
c          print*, itNewton
          call stiff_residl(imeas) ! build rhs and tangent stiffness
          if (timingpr) call timerProfiling(2)
          call CSR_MATLAB (atang,jdiag,jcsr,aload,neqns,1)            
     ...
     +           itNewton,', residual(L2)= ',convRes,
     +           ', increment(L2)= ',convSol
          if (itNewton.gt.100.or.isnan(convRes)) then! reduce the step length to allow convergence
           if (nReduceTimeStepAmplitude .lt. 5) then! this has not been tested yet... JFD 2011-04-12
...
c  modify the step length and restart the computation
     ...
     +          'tolerance can not be reached. Going on as is...'
            exit
           endif !  if (nReduceTime....)
          endif !  if itNewton.qt.30....)
        
c       enddo ! do while (itNewton.lt.1) ! this is to print only 1st newton iteration of forward solve
c        exit ! exit the do while (itNewton.lt.1)
        enddo!  do while (convRes.gt.tol)
        total_primal(imeas,1:npoin,1:mdofn)=primal(1:npoin,1:mdofn)! save the field obtained
      enddo! do while (accumStepL...)
      end
...
c*******************************************************      
      subroutine EnforceInitialCond(imeas)
c     to initialize primal with inital guess for newton iteration
...
c----------------------------------------------------------------
      do ipoin = 1,npoin
       do idofn =1,3
       primal(ipoin,idofn)=init_val(ipoin,idofn)
       enddo ! idofn
      enddo ! ipoin
      return
      end
...
c     increase boundary conditions incremental
      ...
      USE MAINMEM
      implicit none
      integer imeas, iii,jj
      double precision factor2
      integer ipoin,nodfx,idofn,iset
c--------------------------------------------------------
      do ipoin = 1,mpoinbc
        nodfx = ipoin_bc(imeas,ipoin)
        do idofn = 1,ldofn(nodfx)
          if (icode_bc(imeas,ipoin,idofn).ne.0) then
c       ! this applies the difference of the boundary condition and 
c       ! the initial guess, incrementally.
            primal(nodfx,idofn) =init_val(nodfx,idofn)
     $                           +(value_bc(imeas,ipoin,idofn)
     $                           -init_val(nodfx,idofn))*factor2
           endif
        enddo
      enddo

      ! incrementally apply source term, as/along with boundary conditions
      do ipoin = 1,npoin
      do iset = 1,nset_nodal
      dforce(iset,ipoin)=init_val(ipoin,1) + 
     $       (adata_nodal(iset,ipoin)-init_val(ipoin,1))*factor2
      enddo
      enddo
      return
      end
...
c      side for the primal problem
      USE MAINMEM
      implicit none
      integer imeas, ii, jj ! ii, jj for debugging, print estiff 
      integer ielem
      integer nnode, nevab
...
c!$OMP PARALLEL DO
c!$OMP& PRIVATE(ielem,pelem,nnode,nevab,elnods,xelem,elemdata_nodal,
c!$OMP&   uelem,uelem_meas,elemvec_ndofn,estif,eforc,ldnum)
c!$OMP&   TID)
c!$OMP& SHARED(nelem,imeas,lrefn,lnods,nset_nodal,adata_nodal,ldofn,
c!$OMP&   idnum,primal,dual,meas,ndime,coord,mprops,props,mevab,jdiag,
c!$OMP&   jcsr,buildSymmetricMatrices,bc_step)
c!$OMP% DEFAULT(none)
c!$OMP& SCHEDULE(STATIC,20)
c!$OMP& REDUCTION(+:atang,aload)

c     element-level computations
      do ielem = 1,nelem
c       The following lines test openmp, uncomment for debug (don't forget the line above!)
c        TID = OMP_GET_THREAD_NUM()
c        print*,'Thread ',TID,', imeas = ',imeas
...
c      print*,aload(ii)
c      enddo
c       'csr_addnum is called to assemble global stiffnes mat'
...
c     Do-loop computes forceM needed for the following do-loop 
     ...
     $            elemdata_nodal,uelem,uelem_meas)           
      ...
      enddo
      open(unit=121,file="patch_app_fprid.txt",access="append")
      write(121,'(E18.10)') forceM
      close(121)
...
c     $              egrad(iset,inode)! not used anymore
      ...
      print* , "---------------------------------------------"
      print* , "data matching term: ", dataMatch
      print* , "---------------------------------------------"
      print*, "Measured force is " , Gforce
      print*, "Predicted force is" , forceM
      print* , "force matching term: ", forceMatch
      print* , "----------------------------------------------"
      ...
      if (newIt.or.countFGcalls.le.2) then
       if (imeas.eq.nmeas) then
        if (nset_nodal.eq.1) then
          write(16,'(1p,E13.5,1p,E13.5,1p,E13.5,1p,E18.10,1p,E13.5,
...
c-------------------------------------------------------
      ...
      do inode = 1,nnode
        nodei = lnods(ielem,inode) ! global node number
        elno(inode) = lnods(ielem,inode)! nodes numbers in the element
C       commmented this to impliment the stablization formulation 
        do iset = 1,nset_nodal
          elemd(iset,inode) = adata_nodal(iset,nodei)
        enddo
c Following there lines are to impliment the stablization formulation
c        print*,nset_nodal
c        do iset = 1,nset_nodal
c          elemd(iset,inode) = dforce(iset,nodei)
c        enddo
...
c     Post process data with element 307
     ...
     $            elemdata_nodal,uel,uel_meas)           
      ...
      print*,"## pforce is ",pforce
      print*,"## MuFactor is ",MuFactor
      endif
